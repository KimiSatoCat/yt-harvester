...
const TRANSLATIONS = {
  ja: {
    ...
    date_start_year: '開始年',
    date_end_year: '終了年',
    split_period: '期間を分割して進捗・再開を細かくする',
    split_period_help: 'YouTube APIの検索は1クエリあたり最大約500件（最新順）しか返しません。\n\n本ツールは検索を「日付ウィンドウの自動分割」で行います。ある期間の動画が500件を超えて取得しきれない場合，その期間を自動的に半分ずつ分割して再検索し，最終的に指定期間全体を漏れなく収集します（最小1時間まで分割）。そのため，この設定の有無にかかわらず500件制限による取りこぼしは起こりません。\n\nこのチェックボックスと「分割単位」は，進捗表示と中断・再開（クォータ上限到達時など）の区切りの細かさを決めるだけのものです。長期間の収集では「年」または「月」を推奨します。\n\n※ クォータ（API利用上限）とは別の話です。',
    split_unit: '分割単位',
    split_day: '日',
    split_week: '週',
    split_month: '月',
    ...
    quota_search_api_label: '検索API',
    quota_video_channel_label: '動画詳細・チャンネル（推定）',
    quota_comments_label: 'コメント取得（推定）',
    ...
    date_range_warning_nosplit: '長期間の収集です。1クエリ500件の制限は日付ウィンドウの自動分割で回避され，指定期間全体を網羅的に取得します。動画数が多い場合はAPIクォータ（既定10,000/日）に達することがあり，その際は進捗が自動保存され，翌日以降に再開して続きを収集できます。',
    quota_vpq_label: '想定動画数/クエリ',
    quota_vpq_hint: '件（最大500件/クエリ）',
    ...
    quota_autosaved_msg: '途中までのデータはIndexedDBに自動保存済みです。下のいずれかを選んでください。',
    quota_option_download: '途中までをZIP出力して終了する（再開しない）',
    quota_option_save: '保存のみ — クォータ回復後に「途中から再開」で続行（ダウンロードなし）',
    quota_option_download_save: '途中までをZIP出力し、進捗も保持する — バックアップを得つつ後で再開できる（推奨）',
  ...
  en: {
    ...
    date_end_year: 'End Year',
    split_period: 'Split by period (finer progress & resume checkpoints)',
    split_period_help: 'YouTube API search returns at most ~500 results per query (newest first).\n\nThis tool searches using automatic date-window subdivision: whenever a window holds more videos than one query can return, it is split in half by date and searched again (down to a 1-hour minimum), so the entire range is collected with no truncation — regardless of this setting.\n\nThis checkbox and the "Split unit" only control how finely progress is shown and where interrupt/resume checkpoints fall (e.g. when the daily quota is reached). "Year" or "Month" is recommended for long ranges.\n\nNote: this is unrelated to API quota limits.',
    ...
    quota_search_api_label: 'Search API',
    quota_video_channel_label: 'Video details & channels (est.)',
    quota_comments_label: 'Comments (est.)',
    ...
    date_range_warning_nosplit: 'Long collection range. The 500-results-per-query limit is bypassed by automatic date-window subdivision, so the entire range is collected exhaustively. If many videos match, the daily API quota (default 10,000) may be reached — progress is then saved automatically so you can resume collection on a later day.',
    quota_vpq_label: 'Est. videos / query',
    quota_vpq_hint: '(max 500 per query)',
    ...
    quota_autosaved_msg: 'Partial data has been auto-saved to IndexedDB. Choose one of the options below.',
    quota_option_download: 'Export partial data as ZIP and finish (no resume)',
    quota_option_save: 'Save only — resume later after quota resets (no download)',
    quota_option_download_save: 'Export ZIP and keep progress — get a backup and resume later (recommended)',
