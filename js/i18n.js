...
const TRANSLATIONS = {
  ja: {
    ...
    split_period_help: 'YouTube APIの検索は1クエリあたり最大500件（最新順）しか返しません。\n\n例えば2005〜2024年を一括で検索すると，最近の約500件（2020年頃以降）しか収集できません。\n\n「期間を分割して取得する」を有効にすると，選択した単位（年・月など）ごとに別々の検索クエリを実行します。各期間から最大500件ずつ取得できるため，20年分を「年」で分割すると最大20×500=10,000件の収集が可能になります。\n\n※ クォータ（API利用上限）の分割とは関係ありません。',
    split_unit: '分割単位',
    split_day: '日',
    split_week: '週',
    split_month: '月',
  ...
  en: {
    ...
    split_period_help: 'YouTube API search returns at most 500 results per query (newest first).\n\nFor example, searching 2005–2024 without splitting returns only the ~500 most recent videos (around 2020 onward).\n\nEnabling period splitting runs a separate search query for each time unit (year, month, etc.), collecting up to 500 results per period. Splitting 20 years by year allows up to 20 × 500 = 10,000 videos.\n\nNote: this is unrelated to API quota limits.',
